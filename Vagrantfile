...
Vagrant.configure("2") do |config|
    config.vm.define "db" do |db|
      ...
      db.vm.synced_folder ".", "/home/vagrant/app" # sync directoy. 
      db.vm.provision "shell", path: "dbprov.sh", privileged: false #provisin sh path for db
    end
  
    config.vm.define "app" do |app| # define app
      app.vm.box = "ubuntu/bionic64" # state version and os
      app.vm.network "private_network", ip: "192.168.10.100" # configure network
      app.vm.synced_folder ".", "/home/vagrant/app" # change it to your home location 
      app.vm.provision "shell", path: "provision.sh", privileged: false  # provide path for your provision.sh 
    end
  end
